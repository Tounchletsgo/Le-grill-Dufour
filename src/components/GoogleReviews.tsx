...
function Stars({ rating }: { rating: number }) {
  return (
    <span className="gr-stars" aria-label={`${rating}/5`}>
      {[1, 2, 3, 4, 5].map((i) => (
...
function BigStars({ rating }: { rating: number }) {
  ...
  return (
    <span className="gr-big-stars" aria-label={`${rating}/5`}>
      {[1, 2, 3, 4, 5].map((i) => {
        if (i <= full) {
          return (
    ...
    <section className="section gr-section" id="avis">
      <div className="container">
        <div className="section-head">
          <span className="eyebrow">{t("reviews.title")}</span>
          <h2 className="section-title">{t("reviews.subtitle")}</h2>
        ...
        <div className="gr-summary">
          <div className="gr-rating-block">
            ...
            <div className="gr-rating-detail">
              ...
              <span className="gr-rating-count">{config.total_count.toLocaleString(locale === "nl" ? "nl-BE" : "fr-BE")} {t("reviews.reviews")}</span>
            </div>
          </div>
          <div className="gr-google-badge">
            ...
            <span>{t("reviews.googleReviews")}</span>
          </div>
        </div>
